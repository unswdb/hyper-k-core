...
void ParNbrCoreDecomposition(Hypergraph &graph, int numThreads, int computeThreshold) {
    std::cout << "par final " << std::endl;

//    computeThreshold = 3000;
...
void find_best_threshold(Hypergraph &graph, int numThreads) {
    std::unordered_map<double, int> threshold_map;
...
//    use binary search to find the best threshold
    ...
    while (true) {
        int threshold = (max_threshold + min_threshold) / 2;
//        run 3 times to get the average time
        std::vector<double> time_list;
        for (int i = 0; i < 3; ++i) {
            auto time_start = std::chrono::high_resolution_clock::now();
            ParNbrCoreDecomposition(graph, numThreads, threshold);
            auto time_end = std::chrono::high_resolution_clock::now();
            std::chrono::duration<double> diff = time_end - time_start;
            time_list.push_back(diff.count());
            graph.resetCore();
        }
//        remove the max and min time
        auto max_time = std::max_element(time_list.begin(), time_list.end());
        auto min_time = std::min_element(time_list.begin(), time_list.end());
        time_list.erase(std::remove(time_list.begin(), time_list.end(), *max_time), time_list.end());
        time_list.erase(std::remove(time_list.begin(), time_list.end(), *min_time), time_list.end());
        auto sum = std::accumulate(time_list.begin(), time_list.end(), 0.0);
        double mean = sum / static_cast<double>(time_list.size());

        std::cout << "\033[1;31m" << "threshold: " << threshold << " time: " << mean << " s" << "\033[0m" << std::endl;
        threshold_map[mean] = threshold;
        if (mean < prev_time) {
            prev_time = mean;
        ...
        if (max_threshold - min_threshold <= 100) {
            break;
        }
    }
}